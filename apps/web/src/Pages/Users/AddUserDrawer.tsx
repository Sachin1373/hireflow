...
export default function AddUserDrawer({ open, onClose, initialData }: Props) {
  ...
  const onSubmit = async (data: FormInputs) => {
    try {
      if (initialData) {
        await api.patch(`/users/${initialData.id}`, data);
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 4 }}>
        Fill out the details below to invite a new HireFlow member. A temporary
        password will be generated automatically and sent via email.
      </Typography>
        ...
        <Stack spacing={3}>
          ...
          <Box>
            ...
            <Stack direction="row" spacing={4}>
              <FormControlLabel
                control={
                  <Checkbox defaultChecked {...register("permissions.read")} />
                }
              ...
              <FormControlLabel
                ...
                label={
                  <Typography variant="body2">Write / Manage Access</Typography>
                }
              />
            </Stack>
          </Box>
